...
# API_BASE_URL / API2_BASE_URL 必须存在
if [ -z "$API_BASE_URL" ]; then
  echo "ERROR: API_BASE_URL env var is not set"
  exit 1
fi
if [ -z "$API2_BASE_URL" ]; then
  echo "ERROR: API2_BASE_URL env var is not set"
  exit 1
fi
...
export API_BASE_URL
export API2_BASE_URL
export VITE_ENV
envsubst '${API_BASE_URL} ${API2_BASE_URL} ${VITE_ENV}' < "$CONFIG_PATH" > "${CONFIG_PATH}.tmp"
mv "${CONFIG_PATH}.tmp" "$CONFIG_PATH"

echo "Runtime config injected: API_BASE_URL=$API_BASE_URL"
echo "Runtime config injected: API2_BASE_URL=$API2_BASE_URL"
